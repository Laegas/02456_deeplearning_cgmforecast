...
def clarke_error_grid(ref_values, pred_values, title_string, unit):
    ...
    #if max(ref_values) > 400/div or max(pred_values) > 400/div:
    ...
    for i, value in enumerate(ref_values):
        if (value <= 70 and pred_values[i] <= 70) or (pred_values[i] <= 1.2*value and pred_values[i] >= 0.8*value):
            clarke['A'] += 1    #Zone A

        elif (value >= 180 and pred_values[i] <= 70) or (value <= 70 and pred_values[i] >= 180):
            clarke['E'] += 1    #Zone E

        elif ((value >= 70 and value <= 290) and pred_values[i] >= value + 110) or ((value >= 130 and value <= 180) and (pred_values[i] <= (7/5)*value - 182)):
            clarke['C'] += 1    #Zone C
        elif (value >= 240 and (pred_values[i] >= 70 and pred_values[i] <= 180)) or (value <= 175/3 and pred_values[i] <= 180 and pred_values[i] >= 70) or ((value >= 175/3 and value <= 70) and pred_values[i] >= (6/5)*value):
            clarke['D'] += 1    #Zone D
        else:
            clarke['B'] += 1    #Zone B
    ...
    clarke_prob = {}
    for zone in 'ABCDE':
        clarke_prob[zone] = clarke[zone] / len(pred_values)

    return plt, clarke, clarke_prob